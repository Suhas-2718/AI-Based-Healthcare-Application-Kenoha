...
class AssistanceActivity : AppCompatActivity() {
    ...
    private val client = OkHttpClient.Builder()
        ...
        .build()
    private val flaskUrl = "http://172.20.10.13:5001/chat" // replace with your Flask IP
...
//      Adding the below code by commenting the above to make it stack one below another
...
//
//        val shouldAutoScroll = lastVisible == messages.size - 2 || lastVisible == -1
//
//        if (shouldAutoScroll) {
//            recyclerView.scrollToPosition(messages.size - 1)
//        }
//    }
    ...
    private fun addMessage(sender: String, text: String) {
        messages.add(Message(sender, text))
        adapter.notifyItemInserted(messages.size - 1)
        // ❌ no scrollToPosition, no smoothScrollToPosition
    }
